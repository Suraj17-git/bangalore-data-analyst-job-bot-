...
def fetch_jobs(city_names, keywords):
    ...
    try:
        for kw in keywords[:4]:
            ...
            url = f'https://www.linkedin.com/jobs/search/?keywords={q}&location={city_slug}'
            r = requests.get(url, headers=HEADERS, timeout=20)
            r.raise_for_status()
            soup = BeautifulSoup(r.text, 'html.parser')
            for job in soup.select('div.base-card')[:20]:
                ...
                location_el = job.select_one('span.job-search-card__location')
        ...
        print('[warn] LinkedIn fetch failed', e)
    return results